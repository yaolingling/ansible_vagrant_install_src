...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    config.vm.define "dev_ansible" do |target|
        ...
        target.vm.network "forwarded_port", guest: 8080, host: 9102
        target.vm.network "forwarded_port", guest: 5672, host: 9103
        target.vm.network "forwarded_port", guest: 9080, host: 9104
        target.vm.network "forwarded_port", guest: 8443, host: 9105
        ...
        target.vm.provision "shell", inline: <<-SHELL
          ...
          sudo ansible-playbook /vagrant/install_rackhd.yml  
